...
head.addEventListener("click", (e)=>{
    ...
    if (countClick % 2 === 0) rows.sort((a, b)=>{
        const aText = a.cells[index].textContent;
        const bText = b.cells[index].textContent;
        if (index === 0 || index === 1 || index === 2) return bText.localeCompare(aText);
        else if (index === 3 || index === 4) return convertToNumber(bText) - convertToNumber(aText);
    });
    else rows.sort((a, b)=>{
        const aText = a.cells[index].textContent;
        const bText = b.cells[index].textContent;
        if (index === 0 || index === 1 || index === 2) return aText.localeCompare(bText);
        else if (index === 3 || index === 4) return convertToNumber(aText) - convertToNumber(bText);
    });
...
allInputs.forEach((input)=>{
    ...
    else if (input.name === "age" || input.name === "salary") input.type = "number";
    else for(let i = 0; i < 6; i++){
        const option = document.createElement("option");
        if (i === 0) {
            option.textContent = "Tokyo";
            option.selected = true;
        } else if (i === 1) option.textContent = "Singapore";
        else if (i === 2) option.textContent = "London";
        else if (i === 3) option.textContent = "New York";
        else if (i === 4) option.textContent = "Edinburgh";
        else if (i === 5) option.textContent = "San Francisco";
...
button.addEventListener("click", (e)=>{
    ...
    if (nameInput.value !== "" && positionInput.value !== "" && ageInput.value !== "" && salaryInput.value !== 0) {
        ...
        if (salaryInput.value === "") newTr.children[4].innerHTML = "$0";
        else {
            const salaryInputNumber = +salaryInput.value;
            newTr.children[4].innerHTML = `$${salaryInputNumber.toLocaleString("de-DE")}`.replaceAll(".", ",");
        }
